...
<div id="bjui-window">
    <header id="bjui-header">
        <div class="bjui-navbar-header">
            ...
            <span class="web-font" ><?php
                if(isset($configData) && !empty($configData)) {
                    $filename = 'images/'.$configData->logo;
                    if(!empty($configData->logo) && is_file($filename)) {
                        ?><img src="<?php echo $this->request->base.'/'.$filename;?>" height="60"><?php
                    }
        ...
        <nav id="bjui-navbar-collapse">
            <ul class="bjui-navbar-right">
                ...
                <li><a href="#"><?php if(isset($userData)){if(empty($userData['nicheng'])){echo $userData['username'];}else{echo $userData['nicheng'];}}?></a></li>
                <li><a href="<?php echo $this->Url->build(["plugin" => $this->request->params['plugin'], "controller" => "Users", "action" => "logout"]);?>" class="red">&nbsp; 退出</a></li>
                                            ...
                                            <ul class="menu-items" data-faicon="<?php echo $child['parent']->icon;?>" data-tit="<?php echo $child['parent']->name;?>">
                                                ...
                                                if(!empty($child['child'])) {
                                                    foreach($child['child'] as $tmp) {
                                                        ?>
                                                        <li><a href="<?php echo $this->request->base.'/'.$tmp->url;?>" data-reload="true" data-autorefresh="true" data-options="{id:'<?php echo $tmp->rel;?>', faicon:'<?php echo $tmp->icon;?>', fresh:true}"><?php echo $tmp->name;?></a></li>
                                                        <?php
                                                    }
                                                }
                                                ?>
            ...
            <div class="tabsPageHeader">
                <div class="tabsPageHeaderContent">
                    <ul class="navtab-tab nav nav-tabs">
                        <li data-url="<?php echo $this->Url->build(["plugin" => $this->request->params['plugin'], "controller" => "Welcome", "action" => "main"]);?>" data-faicon="home"><a href="javascript:;"><span><i class="fa fa-home"></i> #maintab#</span></a></li>
                    </ul>
                </div>
    ...
    <footer id="bjui-footer">Copyright &copy; <?php if(!empty($configData)) {echo $configData->startyear.' '.$configData->name;}?>
    </footer>
</div>
</body>
</html>